...
from . models import Post, LikeSystem, Tag, Comment, Profile, News, Variable, Vote
...
class PostDetail(DetailView):
	...
	def get_context_data(self, **kwargs):
		...
		profiles = [Profile.objects.get(user=comment.author) for comment in comments]
		...
		context['comments_count'] =  len(Comment.objects.filter(post=self.object))
...
class LikeSystemView(LoginRequiredMixin,View):
	...
	def post(self, request, pk):
		data=dict() 
		content_object = self.model.objects.get(pk=pk) # получаем экземпляр модели с которой работаем Post/Comment
		...
		try:
			...
			if likedislike.action != self.action_type:  # если ранее данным пользователем производились дейтсвия
														# над этим обьектом 

				likedislike.action = self.action_type	# если ранее лайкал, а теперь дизлайкает, или наоборот
				likedislike.save(update_fields=['action']) # изменяем поле на нужное и сохраняем
			...
			content_object.likesystem.create(action= self.action_type, user = request.user )
...
class CommentView(LoginRequiredMixin,View):
	...
	def post(self, request, pk, *args, **kwargs):
		...
		content_object = self.model.objects.get(pk=pk) # получаем экземпляр модели с которой работаем Post/News
		...
		if form.is_valid():
			...
			if Comment.check_spam(author):
				...
				comment = Comment.objects.create(author=author, content_type = content_type, object_id = content_object.id, message=message)
				comments_count = len(Comment.objects.filter(content_type = content_type, object_id = content_object.id))
				new_comment = render_to_string('new_comment.html', {'comment': comment})
				data = {'new_comment':new_comment, 'comments_count':comments_count}
...
class DeleteCommentView(LoginRequiredMixin,SingleObjectMixin, View):
	...
	def post (self, request, *args, **kwargs):
		...
		if self.request.user == self.object.author or request.user.is_staff == True :
			content_type = self.object.content_type
			self.object.delete()
			comments_count = len(Comment.objects.filter(content_type=self.object.content_type,
	...
	def post(self, request, *args, **kwargs):
		...
		if form.is_valid():
			...
			if Comment.check_spam(author):
				...
				headline=self.object.content_object.headline #название поста/новости
				link_to =self.object.content_object.get_absolute_url() #ссылка на пост/новость
				original_text=self.object.message # cообщение на которое отвечают
				request_user_name = request.user.first_name+' '+request.user.last_name #кто отвечает
				...
				answer = Comment.objects.create(author=author,
												content_type=self.object.content_type,
												object_id= self.object.object_id,
												message=message,
												answer=self.object,
				...
				if profile.subscribe_answers:
					...
					try:
						answer_notify_email.delay(user.id, headline, link_to, original_text, request_user_name)
					except:
						pass
						

				comments_count = len(Comment.objects.filter(content_type=self.object.content_type,object_id= self.object.object_id))
				new_answer = render_to_string('new_answer.html', {'answer': answer
					})
				data = {'new_answer':new_answer, 'comments_count':comments_count, 'status':status}
				return JsonResponse(data, safe=False)
			else:
				return JsonResponse({'spam':True},status=400)
		else:
...
class VotesView(LoginRequiredMixin,View):
	...
	def post(self, request, pk):
		...
		if  news.question and news.question_is_active:
			...
			try:
				vote = Vote.objects.get(news=news,user=request.user) #проверка голосовал ли ранее в этом опросе

				if  vote.variable == variable:  # если голосовал за этот вариант и снова пытается проголосовать
					vote.delete()
		
				else: #  ранее голосовал за другой вариант
					vote.delete()
					Vote.objects.create(news=news,variable=variable, user = request.user )

			except:
				Vote.objects.create(news=news, variable=variable, user = request.user ) # ранее не голосовал
