...
class Profile(models.Model):
	...
	avatar_url = models.CharField(max_length=200, default='/media/avatars/default-XRANGE202418022019.jpg')
...
class Post(models.Model):
	...
	label_img =  ProcessedImageField(upload_to = path_label_img, default='label_img/images.jpeg', 
	...
	likesystem  = GenericRelation('LikeSystem', related_query_name='post')  #like and dislike objects

	comments  = GenericRelation('Comment', related_query_name='post')  #comments
	...
	def comment_count(self):
		comment = Comment.objects.filter(content_type=ContentType.objects.get_for_model(self), object_id=self.id)
...
class Comment(models.Model):
	...
	likesystem  = GenericRelation('LikeSystem', related_query_name='comment')  #like and dislike objects
...
class News(models.Model):
	...
	comments  = GenericRelation('Comment', related_query_name='news')  #comments

	created = models.DateTimeField(default=timezone.now)

	question = models.BooleanField(default=False)

	question_headline = models.CharField(max_length=1000, blank=True, null=True)
	...
	def total_votes(self):
		variables = Variable.objects.filter(news=self) # все голосавания по данной новости
		total=sum([Vote.objects.filter(variable=i).count() for i in variables])
		return total


	def comment_count(self):
		comments = Comment.objects.filter(content_type=ContentType.objects.get_for_model(self), object_id=self.id)
...
class Vote(models.Model):
	...
	variable = models.ForeignKey('Variable', on_delete=models.CASCADE, related_name='vote')

	news = models.ForeignKey('News', on_delete=models.CASCADE, related_name='vote')
...
def listenPostSignal(sender, instance, created, **kwargs):
	if created:
		newPublicationPost.delay(instance.headline, instance.get_absolute_url())
post_save.connect(listenPostSignal, sender=Post, dispatch_uid="listenPostSignal")
#==================================================================================================#
#==================================================================================================#
def listenNewsSignal(sender, instance, created, **kwargs):
	if created:
		newPublicationNews.delay(instance.headline, instance.get_absolute_url())
post_save.connect(listenNewsSignal, sender=News, dispatch_uid="listenNewsSignal")
#==================================================================================================#